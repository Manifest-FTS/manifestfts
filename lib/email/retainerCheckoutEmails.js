...
function buildInternalPlainText(fields) {
  ...
  return [
    'New retainer subscription (checkout completed)',
    '',
    ...
    `Monthly total: ${monthlyTotal || 'N/A'}`,
    '',
    'Project brief:',
    brief || 'N/A',
    '',
...
<head>
  ...
  <title>New retainer subscription</title>
</head>
        ...
        <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:12px;overflow:hidden;border:1px solid #e4e7ec;box-shadow:0 4px 24px rgba(16,24,40,0.08);">
          <tr>
            <td style="padding:28px 32px 20px 32px;border-bottom:1px solid #eef2f6;background:linear-gradient(180deg,#fafbfc 0%,#ffffff 100%);">
              ...
              <p style="margin:16px 0 0 0;font-size:11px;letter-spacing:0.12em;text-transform:uppercase;color:#667085;font-family:system-ui,-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">Monthly retainer</p>
              <h1 style="margin:8px 0 0 0;font-size:22px;line-height:1.25;color:#101828;font-family:system-ui,-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;font-weight:600;">New subscription confirmed</h1>
              <p style="margin:10px 0 0 0;font-size:15px;line-height:1.5;color:#475467;font-family:system-ui,-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">Someone completed Stripe Checkout for a retainer. Summary and Stripe references are below.</p>
            </td>
          </tr>
          <tr>
                ...
                <tr>
                  <td style="padding:16px 18px;font-family:system-ui,-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
                    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="font-size:14px;color:#101828;">
                      ...
                      <tr><td style="padding:8px 0 4px 0;color:#667085;border-top:1px solid #e4e7ec;">Monthly total</td><td style="padding:8px 0 4px 0;text-align:right;font-size:18px;font-weight:700;color:#101828;border-top:1px solid #e4e7ec;">$${escapeHtml(monthlyTotal)}</td></tr>
                    </table>
                  </td>
                </tr>
              </table>
...
function buildCustomerPlainText(fields, origin) {
  const { customerName, selectedCommitment, monthlyTotal } = fields;
  const name = customerName ? `${customerName},` : 'Hi there,';
  return [
    `${name}`,
    '',
    'Thank you for subscribing to a monthly retainer with Manifest FTS.',
    '',
...
function buildCustomerHtml(fields, origin) {
  const logoUrl = `${DEFAULT_SITE_ORIGIN}/assets/imgs/logo.svg`;
  const capabilitiesUrl = `${origin}/capabilities`;
  const { customerName, selectedCommitment, monthlyTotal, brief } = fields;
...
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>Your retainer is confirmed</title>
</head>
<body style="margin:0;padding:0;background-color:#f4f6f8;font-family:Georgia,'Times New Roman',serif;-webkit-font-smoothing:antialiased;">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#f4f6f8;padding:24px 12px;">
    <tr>
      <td align="center">
        <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:12px;overflow:hidden;border:1px solid #e4e7ec;">
          ...
          <tr>
            <td style="padding:28px 32px 8px 32px;font-family:system-ui,-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
              ...
              <p style="margin:0 0 16px 0;font-size:15px;line-height:1.6;color:#475467;">Thank you for starting a monthly retainer with Manifest FTS. Your payment was received and your subscription is active.</p>
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#f8fafc;border-radius:10px;border:1px solid #e4e7ec;margin-bottom:16px;">
                <tr>
                  <td style="padding:18px 20px;font-size:15px;color:#101828;">
                    <strong style="display:block;margin-bottom:8px;">Your plan</strong>
                    ${escapeHtml(selectedCommitment) || '—'}<br/>
                    <span style="font-size:20px;font-weight:700;color:#3f8077;margin-top:8px;display:inline-block;">$${escapeHtml(monthlyTotal)}<span style="font-size:14px;font-weight:500;color:#667085;"> / month</span></span>
                  </td>
                </tr>
              </table>
        ...
        <p style="margin:14px 0 0 0;font-size:11px;color:#98a2b3;font-family:system-ui,-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">You may also receive receipts from Stripe for subscription charges.</p>
      </td>
    </tr>
  </table>
</body>
...
export function buildInternalRetainerSubscriptionEmail(session) {
  ...
  return {
    subject: 'New retainer subscription · Manifest FTS',
...
export function buildCustomerRetainerConfirmationEmail(session) {
  ...
  const fields = extractSessionFields(session);
  return {
    subject: 'Your Manifest FTS retainer is confirmed',
